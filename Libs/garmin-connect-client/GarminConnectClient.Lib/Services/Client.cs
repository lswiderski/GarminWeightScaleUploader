...
namespace GarminConnectClient.Lib.Services
{
    ...
    public class OAuth1
    {
        ...
        {
            return method + "&" + url + "&" + string.Join("&", oauthParams.OrderBy(p => p.Key).Select(p => $"{p.Key}={p.Value}"));
        }

        public string Signature(string consumerSecret, string tokenSecret, string signatureBase)
        {
            var encoding = new ASCIIEncoding();
            string key = consumerSecret + "&";//+ (string.IsNullOrEmpty(tokenSecret) ? "" : Uri.EscapeDataString(tokenSecret));
        ...
        public string Nonce(int length = 11)
        {
        ...
        public async Task<string> Request(string url, string oAuthHeader)
        {
            using (var httpClient = new HttpClient())
            {
                httpClient.DefaultRequestHeaders.Add("user-agent", "com.garmin.android.apps.connectmobile");
                ...
                HttpResponseMessage httpResp = await httpClient.GetAsync(url);
